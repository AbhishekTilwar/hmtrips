import {
  require_react_dom
} from "./chunk-FBUWX7EP.js";
import {
  require_react
} from "./chunk-3FNYQ5XX.js";
import {
  __async,
  __toESM
} from "./chunk-YEWLISYD.js";
...
function createRouter(init) {
  ...
  function startNavigation(historyAction, location, opts) {
    return __async(this, null, function* () {
      ...
      if (opts && opts.pendingError) {
        ...
        let actionResult = yield handleAction(request, location, opts.submission, matches, fogOfWar.active, {
  ...
  function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    return __async(this, null, function* () {
      ...
      if (isFogOfWar) {
        let discoverResult = yield discoverRoutes(matches, location.pathname, request.signal);
      ...
      if (!actionMatch.route.action && !actionMatch.route.lazy) {
        ...
        let results = yield callDataStrategy("action", state, request, [actionMatch], matches, null);
  ...
  function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult) {
    return __async(this, null, function* () {
      ...
      if (redirect3) {
        yield startRedirectNavigation(request, redirect3.result, true, {
          replace: replace2
        });
        return {
          shortCircuited: true
  ...
  function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
    return __async(this, null, function* () {
      ...
      if (isFogOfWar) {
        let discoverResult = yield discoverRoutes(requestMatches, new URL(fetchRequest.url).pathname, fetchRequest.signal, key);
        if (discoverResult.type === "aborted") {
          return;
        } else if (discoverResult.type === "error") {
      ...
      let actionResults = yield callDataStrategy("action", state, fetchRequest, [match], requestMatches, key);
  ...
  function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, preventScrollReset, submission) {
    return __async(this, null, function* () {
      ...
      if (isFogOfWar) {
        let discoverResult = yield discoverRoutes(matches, new URL(fetchRequest.url).pathname, fetchRequest.signal, key);
        if (discoverResult.type === "aborted") {
          return;
        } else if (discoverResult.type === "error") {
          setFetcherError(key, routeId, discoverResult.error, {
      ...
      fetchControllers.set(key, abortController);
      let originatingLoadId = incrementingLoadId;
      let results = yield callDataStrategy("loader", state, fetchRequest, [match], matches, key);
  ...
  function startRedirectNavigation(request, redirect3, isNavigation, _temp2) {
    return __async(this, null, function* () {
      let {
        submission,
        fetcherSubmission,
  ...
  function callDataStrategy(type, state2, request, matchesToLoad, matches, fetcherKey) {
    return __async(this, null, function* () {
      let results;
      let dataResults = {};
      try {
        results = yield callDataStrategyImpl(dataStrategyImpl, type, state2, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2);
      } catch (e) {
  ...
  function callLoadersAndMaybeResolveData(state2, matches, matchesToLoad, fetchersToLoad, request) {
    return __async(this, null, function* () {
      ...
      let fetcherResultsPromise = Promise.all(fetchersToLoad.map((f) => __async(this, null, function* () {
        if (f.matches && f.match && f.controller) {
          let results = yield callDataStrategy("loader", state2, createClientSideRequest(init.history, f.path, f.controller.signal), [f.match], f.matches, f.key);
      ...
      let fetcherResults = (yield fetcherResultsPromise).reduce((acc, r) => Object.assign(acc, r), {});
      yield Promise.all([resolveNavigationDeferredResults(matches, loaderResults, request.signal, currentMatches, state2.loaderData), resolveFetcherDeferredResults(matches, fetcherResults, fetchersToLoad)]);
      return {
        loaderResults,
        fetcherResults
      };
...
function callDataStrategyImpl(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, requestContext) {
  return __async(this, null, function* () {
    ...
    let dsMatches = matches.map((match, i) => {
      ...
      let resolve = (handlerOverride) => __async(this, null, function* () {
    ...
    let results = yield dataStrategyImpl({
      matches: dsMatches,
      request,
...
function callLoaderOrAction(type, request, match, loadRoutePromise, handlerOverride, staticContext) {
  return __async(this, null, function* () {
    ...
    let runHandler = (handler) => {
      ...
      let handlerPromise = (() => __async(this, null, function* () {
        try {
  ...
  return __async(this, null, function* () {
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      if (isDeferredResult(result) && isRevalidatingLoader) {
        yield resolveDeferredData(result, signal, false).then((result2) => {
          if (result2) {
  ...
  return __async(this, null, function* () {
    for (let index = 0; index < revalidatingFetchers.length; index++) {
      ...
      if (isDeferredResult(result)) {
        ...
        yield resolveDeferredData(result, controller.signal, true).then((result2) => {
          if (result2) {
            results[key] = result2;
          }
        });
